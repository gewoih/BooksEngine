...
Схема — из EF-миграций (`dotnet ef migrations script`), БД — отдельная booksengine_test
в контейнере из docker-compose. Нет Postgres или dotnet — тесты пропускаются.
"""
import json
import shutil
import subprocess
...
def test_removing_work_with_app_rating_fails_and_rolls_back(dsn, tmp_path):
    ...
    with psycopg.connect(dsn) as conn:
        ...
        conn.execute("INSERT INTO ratings (user_id, work_id, value) "
                     "SELECT u.id, w.id, 9 FROM app_users u, works w WHERE w.title = 'Box Set #1-3'")
