...
def test_profile_input_takes_ratings_in_core(tmp_path):
    prof = tmp_path / "profile.csv"
    pd.DataFrame({"title": ["Дюна", "Эмма", "Вне", "Улисс", "Возлюбленная"], "rating": [2, 1, 4, 1, None],
                  "status": ["read", "dnf", "read", "read", "read"],
                  "goodreads_work_id": [1, 3, 999, 5, 6]}).to_csv(prof, index=False)
    x = ex._profile_input(prof, np.array([1, 3, 5, 6]))
    # вне ядра и без оценки — пропуск
    assert x.toarray().tolist() == [[2.0, 1.0, 1.0, 0.0]]


def _run(cores, split, tmp, name, model_dir):
    prof = tmp / "profile.csv"
    pd.DataFrame({"title": ["Дюна", "Эмма", "Вне"], "rating": [5, None, 4],
