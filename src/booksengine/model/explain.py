...
def layers_contributions(layers, x: sp.csr_matrix, cols: np.ndarray,
    ...
    """Вклады книг входа в балл слоёв «толпа + вкус» (`Layers.score`) книг cols и постоянная часть (вкус:
    «как книгу обычно оценивают»). Сумма вкладов + постоянная = балл, точно. Возвращает (вход, вклады, постоянная)."""
    v = layers.variant
    if v.crowd == "like":
    ...
    const = np.zeros(len(cols))
    if v.taste_weight:
        ct, kt = taste_contributions(layers.taste, x, cols, m.ease.top_cols)
        c, const = c + v.taste_weight * ct, v.taste_weight * kt
    return in_cols, c, const
