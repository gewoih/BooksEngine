"""EASE^R: B = I − P·diag(1/diag P), P = (XᵀX + λI)⁻¹, diag B = 0.
...
class EASE:
    ...
    def fit(self, train: RatingMatrix) -> None:
        ...
        # Одна плотная копия n × n на всё обучение (пик на 30 000 книг — ~12 ГБ из 16): LAPACK обращает на месте
        # только матрицу, лежащую по столбцам, а построчную G scipy молча копирует. G симметрична точно (целые
        # счётчики во float32), поэтому G.T — та же матрица по столбцам: результат побитово тот же, что inv(G).
...
class EASELike(EASE):
    """Толпа «ценность» — целится в оценку: вход — звёзды с весами `weights` (по умолчанию 1★ −2 … 5★ +2, как во
    входе EASE смеси; подбор — `layers.tune_like`), цель — «оценка − 3» у прочитанных, 0 у непрочитанных, то есть
    ровно «ценность топа», по которой выбирается выдача. EASE учился «прочтёт ли»; этот — «сколько ценности даст».
