...
нынешнего (парная разница: интервал касается нуля или падение ≤ 0.005) и Low@20 не выше (так же).
...
def allowed(row: dict) -> bool:
    """NDCG@20 не хуже нынешнего и Low@20 не выше — с точностью до шума (журнал решений, «слой вкуса»)."""
    g = row["groups"]["all"]
    nd, lo = g["ndcg20_diff"], g["low20_diff"]
    return (nd["hi"] >= 0 or nd["mean"] >= -NOISE) and (lo["lo"] <= 0 or lo["mean"] <= NOISE)
...
def report(res: dict) -> str:
    ...
    lines = [f"# Слои «толпа + вкус» ({'валидация' if res['stage'] == 'val' else 'тест'}, TODO п. 37, шаг 2)", "",
             ...
             f"в скобках 95% интервал. Допустим — NDCG@20 не хуже и Low@20 не выше с точностью до шума ({NOISE}); "
