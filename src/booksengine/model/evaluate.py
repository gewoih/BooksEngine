"""Стенд моделей `MODELS`: перебор настроек на валидации по NDCG@20, финальный замер лучшей на тесте.

Слои и толпа «ценность» выбираются по ценности топа — в `layers.py`, не здесь.
"""
import json
import time
...
MODELS: dict[str, tuple[type, list[tuple[dict, list[dict]]]]] = {
    ...
    # 128 координат (256 — та же точность вдвое дороже, выдача гуще по автору), λ = 0.1, α = 1;
    ...
    "knn": (ItemKNN, [({"beta": b, "k_max": 200}, [{"k": k, "normalize": False} for k in (3, 5, 7, 10, 20, 50)])
                      for b in (0.0, 50.0)]),
    # смесь готовых als_neg и ease (вход EASE по оценке −2/−1/0/1/2); 0 — чистый EASE, 1 — чистый ALS
    ...
    # 30 000 книг вместо 20 000 — все книги профилей из 20–50K лежат до 30K (`booksengine ease-size`)
...
def run_eval(model, hold: Holdout, batch: int = 500) -> tuple[pd.DataFrame, float]:
    """Топ-K для каждого пользователя группы пачками (≤ 500 × книги ядра float32 в памяти) и метрики."""
