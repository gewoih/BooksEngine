...
Тест и валидация набираются поровну по этапам пути читателя (`TEST_PER_BUCKET`, `VAL_PER_BUCKET`): границы —
удвоения числа оценок, на каждом следующем этапе модель знает о человеке вдвое больше. Поровну — значит, судья
(среднее по людям) весит каждый этап одинаково. Люди с 1000+ оценок — другой режим (перенос всей библиотеки разом
или больше 8 книг в месяц все годы Goodreads): в проверку не берутся, в обучении остаются. Валидация — по 1000 на этап:
у малых профилей спрятано 4–7 книг, у многих не угадано ни одной, и в качество они не входят. Отбор — по стабильному
хэшу внешнего id (`hash01`) с фиксированным порогом на этап, а не случайной перестановкой всего пула: порог не
зависит от того, сколько сейчас всего людей в ядре, поэтому человек не покидает тест только из-за того, что очистка
убрала кого-то другого.
"""
import hashlib
import json
...
SEED = 20260923
HIDDEN_SHARE = 0.2
BUCKETS = ((20, "20-39"), (40, "40-79"), (80, "80-159"), (160, "160-319"), (320, "320-999"), (1000, "1000+"))
OUTSIDE = "1000+"
BUCKET_ORDER = [name for _, name in BUCKETS if name != OUTSIDE]
GROUPS = ("val", "test")

# Размер этапа в ядре (20, 100) по замеру 2026-09-25 — калибровка порога хэша.
...
BUCKET_POOL_SIZE = {"20-39": 149_000, "40-79": 155_000, "80-159": 138_000, "160-319": 93_000, "320-999": 56_000}
TEST_PER_BUCKET = dict.fromkeys(BUCKET_ORDER, 1_200)
VAL_PER_BUCKET = dict.fromkeys(BUCKET_ORDER, 1_000)


def bucket_of(n: np.ndarray) -> np.ndarray:
    """Этап по полному числу оценок пользователя."""
    n = np.asarray(n)
    ...
    return np.array([name for _, name in BUCKETS])[np.searchsorted(edges, n, side="right") - 1]
