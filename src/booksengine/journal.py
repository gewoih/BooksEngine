...
датасете: порог «угадано ≥ 90%» (`layers.GUARD`) выбран без замера и задаёт вес вкуса. Поэтому отдельно — книги,
...
def _debug_lines(read: pd.DataFrame, bold: pd.DataFrame, rated: pd.Series) -> list[str]:
    """Проверка порога «угадано ≥ 90%»: прочитанное из советов — поднятое вкусом против поставленного толпой, и
    ...
    out = ["", "Порог «угадано ≥ 90%» (вес вкуса): если поднятое вкусом и прочитанное из «смелой» выдачи оценено не "
