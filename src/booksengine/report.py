...
""")
    ...
    w("\n`rating = 0` означает не «ноль баллов», а отсутствие оценки: книга на полке (to-read), либо прочитана "
      "без оценки. Это другой сигнал (интерес, а не оценка), поэтому он хранится отдельно в `shelf_events`.\n")
    ...
    w(f"- **`rating = 0` → `shelf_events`: {_n(sp['detail']['shelf_rows'])} записей.** В модель оценок они не идут, "
      f"но сохраняются: после сворачивания до произведений это {_n(sp['detail']['shelf_pairs_work_level'])} пар; "
      f"в итоговом файле — {_n(out['shelf_events']['rows'])} пар для оставшихся пользователей.")
    ...
    w(f"- **≥ {kc['min_user_ratings']} у пользователя**: на этапе 3 в отложенную выборку уходит 20% оценок, "
      ...
      "проверять по одной.")
    w(f"- **≥ {kc['min_work_ratings']} у произведения**: произведения с 10–19 оценками — "
      "это около 180 тыс. позиций при ~2% оценок. Для них похожесть и эмбеддинги статистически ненадёжны, "
      "а с числом произведений растут размер модели и время обучения. Переход от (10, 10) к (10, 20) убирает "
      "35% произведений ценой 2.5% оценок. Порог 50 урезал бы каталог CF до 171 тыс. позиций и отрезал бы "
      "нишевые книги, которые как раз и интересно рекомендовать.\n")
...
| `shelf_events` | неявный сигнал: произведение на полке / прочитано без оценки |
